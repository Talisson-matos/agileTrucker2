...
app.post('/upload', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const texto = pdfData.Pages.map((page) => page.Texts.map((text) => decodeURIComponent(text.R[0].T)).join(' ')).join(' ');
...
function extrairCNPJ(texto, posicao) {
    const matches = texto.matchAll(/\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}/g);
    const cnpjs = Array.from(matches).map(match => match[0].replace(/[.\-\/\s]/g, '') // Remove ponto, traço, barra e espaço
    );
...
function extrairCNPJPagadorFrete(texto, cnpjRemetente, cnpjDestinatario) {
